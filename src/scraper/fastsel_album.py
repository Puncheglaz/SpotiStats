...
def get_js_func(album_id):
    return f'''
    fetch("https://api-partner.spotify.com/pathfinder/v1/query?operationName=getAlbum&variables=%7B%22uri%22%3A%22spotify%3Aalbum%3A{album_id}%22%2C%22locale%22%3A%22%22%2C%22offset%22%3A0%2C%22limit%22%3A50%2C%22enableAssociatedVideos%22%3Afalse%7D&extensions={template_args['extension']}", {{
      "headers": {{
...
def get_net_data():
    ...
    for log in filter(log_filter, logs):
        if 'operationName=getAlbum&' in str(log):
            print(log)
...
def fetch_album(album_id):
    ...
    await fetch("https://api-partner.spotify.com/pathfinder/v1/query?operationName=getAlbum&variables=%7B%22uri%22%3A%22spotify%3Aalbum%3A{album_id}%22%2C%22locale%22%3A%22%22%2C%22offset%22%3A0%2C%22limit%22%3A50%2C%22enableAssociatedVideos%22%3Afalse%7D&extensions={template_args['extension']}", {{
      "headers": {{
        "accept": "application/json",
        "accept-language": "en",
        "app-platform": "WebPlayer",
